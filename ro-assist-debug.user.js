...
(function () {
  ...
  var VERSION_RE = /\?([0-9.]+)/;
  var VER = "2.8.3-debug"; // 面板标题/加载提示/日志统一版本号（bump 时与 @version 同步改）
  ...
  var PAGE_HTML = {
    ...
    system: '' +
      ...
      '<details style="margin-top:4px"><summary style="cursor:pointer;color:#7c1d1d;font-size:12px">诊断 · 关窗取证（v2.8.3-debug）</summary>' +
      '<div class="row"><button id="dsh-trace-copy" style="flex:0 0 auto">复制日志</button>' +
      '<button class="ghost" id="dsh-trace-clear" style="flex:0 0 auto">清空</button>' +
      '<span class="st" id="dsh-trace-status" style="font-size:10px;word-break:break-all">自动记录事件与窗口开关；复发后点「复制日志」发我</span></div>' +
      '<div class="st" style="font-size:10px;color:#555">仅记录 touch/click/键/窗口显隐，不改任何游戏逻辑</div></details>' +
  ...
  // ---------------- V2.8.3-debug 诊断：关窗瞬间取证（只记录，不改逻辑）----------------
  try {
    var __T = { boot: Date.now(), href: location.href, ua: navigator.userAgent, evts: [], muts: [], wins: {}, snap: null };
    window.__dshTrace = __T;
    function __tTag(el) {
      try {
    ...
    function __tPush(ev) {
      try {
        __T.evts.push({ dt: Date.now() - __T.boot, type: ev.type, t: ev.isTrusted ? 1 : 0, pt: ev.pointerType || '', tg: __tTag(ev.target), x: Math.round(ev.clientX || 0), y: Math.round(ev.clientY || 0) });
        if (__T.evts.length > 300) __T.evts.shift();
      } catch (e) {}
    }
    ["pointerdown","pointerup","pointercancel","touchstart","touchend","touchcancel","mousedown","mouseup","click","dblclick","keydown","wheel","contextmenu"].forEach(function (t) {
      try { document.addEventListener(t, __tPush, true); } catch (e) {}
    });
    function __tMuts(recs) {
      try {
        for (var i = 0; i < recs.length; i++) {
          var r = recs[i], tg = __tTag(r.target);
          if (!/win|mask|panel|page|menu|dialog|pop/i.test(tg + ' ' + (r.target && r.target.id || ''))) continue;
          __T.muts.push({ dt: Date.now() - __T.boot, type: r.type, tg: tg, att: r.attributeName || '', added: r.addedNodes && r.addedNodes.length ? r.addedNodes.length : 0, removed: r.removedNodes && r.removedNodes.length ? r.removedNodes.length : 0 });
          if (__T.muts.length > 200) __T.muts.shift();
        }
      } catch (e) {}
    }
    var __tObs = null;
    function __tObserve() {
      try {
        var root = document.getElementById('vbk') || document.body;
        __tObs = new MutationObserver(__tMuts);
        __tObs.observe(root, { subtree: true, attributes: true, childList: true, attributeFilter: ['class', 'style', 'display'] });
      } catch (e) {}
    }
    if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', __tObserve); else __tObserve();
    function __tPoll() {
      try {
        var cands = document.querySelectorAll('#vbk [id],[class*="win"],[class*="mask"],[class*="page"],[class*="dialog"],[class*="menu"],[class*="pop"]');
        if (!cands.length) cands = document.querySelectorAll('[class*="win"],[class*="mask"],[class*="dialog"],[class*="menu"],[class*="pop"]');
        for (var i = 0; i < cands.length && i < 400; i++) {
          var el = cands[i], vis = false, area = 0;
          try {
            var st = getComputedStyle(el), r = el.getBoundingClientRect();
            vis = st.display !== 'none' && st.visibility !== 'hidden' && r.width > 60 && r.height > 60;
            area = Math.round(r.width * r.height);
          } catch (e) {}
          var key = __tTag(el), prev = __T.wins[key];
          if (vis && !prev) __T.wins[key] = { open: Date.now() - __T.boot, area: area, count: 1 };
          else if (!vis && prev && prev.open) {
            var closedAt = Date.now() - __T.boot;
            __T.snap = { win: key, openedAt: prev.open, closedAt: closedAt, delay: closedAt - prev.open, lastEvents: __T.evts.slice(-20), lastMuts: __T.muts.slice(-8) };
            delete __T.wins[key];
          } else if (vis && prev) prev.count++;
        }
      } catch (e) {}
    }
    setInterval(__tPoll, 250);
    setTimeout(function () {
      try {
        var bt = $id && $id("dsh-trace-copy");
        if (bt) bt.addEventListener("click", function () {
          try {
            var log = { ver: VER, href: __T.href, ua: __T.ua, evts: __T.evts, muts: __T.muts, snap: __T.snap, wins: __T.wins };
            var t = JSON.stringify(log, null, 1);
            ...
            var st = $id("dsh-trace-status");
            if (st) st.textContent = "已复制 " + t.length + " 字符（" + __T.evts.length + " 事件，" + (__T.snap ? "已有关窗快照" : "尚无快照") + "）";
          } catch (e3) {}
        });
        var cl = $id && $id("dsh-trace-clear");
        if (cl) cl.addEventListener("click", function () { __T.evts = []; __T.muts = []; __T.snap = null; __T.wins = {}; });
      } catch (e) {}
    }, 500);
  } catch (e) {}
  ...
  function checkDefense(mobs, ent) {
    try {
      ...
      if ($id("dsh-z-sit").checked && life) {
        ...
        if (shouldSit && sitBtn && !defSnap.sitting && isActFreeOnline("sit")) {
          try { sitBtn.click(); } catch (e) {}
          sitSince = Date.now(); sitHpAt = hpPct2;
          lockAct("sit", 60000);
        }
        else if (!shouldSit && standBtn && hpPct2 > hi && spPct2 > sHi) {
          try { standBtn.click(); } catch (e) {}
          sitSince = 0; sitHpAt = -1;
          if (actLock.act === "sit") actLock.act = null;
        }
        // 看门狗：坐下 30s 血未回升（坐不住）→ 站起；战斗图被围 → 下周期并入瞬移链
        if (defSnap.sitting && sitSince && (now - sitSince > 30000) && hpPct2 <= (sitHpAt >= 0 ? sitHpAt + 2 : hpPct2)) {
